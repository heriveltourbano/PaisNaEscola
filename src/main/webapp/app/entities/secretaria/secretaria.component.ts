...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/secretaria'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Secretaria) {
        return item.id;
    }
    registerChangeInSecretarias() {
        this.eventSubscriber = this.eventManager.subscribe('secretariaListModification', (response) => this.loadAll());
    }
